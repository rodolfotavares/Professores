...
function NavIcon({ href, label, icon }: { href: string; label: string; icon: IconName }) {
  return (
    <Link href={href} aria-label={label} title={label} className={icon === 'bot' ? 'lumi-nav-link' : undefined}>
      <Icon name={icon} />
      <span>{label}</span>
    </Link>
  );
...
type IconName = 'home' | 'calendar' | 'users' | 'tasks' | 'chart' | 'message' | 'support' | 'smart' | 'bot';
...
function Icon({ name }: { name: IconName }) {
  ...
  if (name === 'bot') return <img className="lumi-nav-avatar" src="/lumina-bot-avatar.png" alt="" aria-hidden="true" />;
