...
Object.assign(dictionary, {
  ...
  'Meus': 'My',
  'Minhas': 'My',
  'Professor particular': 'Private teacher',
  'Aula particular': 'Private class',
  'Abrir aula online': 'Open online class',
  'Iniciar aula': 'Start class',
  'Finalizar aula': 'Finish class',
  'Confirmar aula': 'Confirm class',
  'Adicionar ao Google Agenda': 'Add to Google Calendar',
  'Nova data': 'New date',
  'Novo horário': 'New time',
  'Novo horario': 'New time',
  'Ações da aula': 'Class actions',
  'Acoes da aula': 'Class actions',
  'Link da aula': 'Class link',
  'Link da aula disponível': 'Class link available',
  'Link da aula ainda não disponível': 'Class link not available yet',
  'Matéria não definida': 'Subject not set',
  'Sem próxima aula': 'No next class',
  'Nenhuma aula agendada': 'No class scheduled',
  'Próximas aulas': 'Upcoming classes',
  'Aulas de hoje': 'Today classes',
  'Aulas registradas': 'Registered classes',
  'Ver histórico': 'View history',
  'Ver histórico da aula': 'View class history',
  'Histórico inteligente': 'Smart history',
  'Evolução do aluno': 'Student progress',
  'Evolução positiva': 'Positive progress',
  'Pontos de atenção': 'Attention points',
  'Aulas analisadas': 'Analyzed classes',
  'Tendência': 'Trend',
  'Parabéns!': 'Congratulations!',
  'Continue assim!': 'Keep going!',
  'Aguardando relatórios': 'Waiting for reports',
  'Preparar dúvidas': 'Prepare questions',
  'Revisar a última aula': 'Review the last class',
  'Conteúdo da próxima aula': 'Next class content',
  'Quando o professor marcar uma aula, ela aparecerá aqui.': 'When the teacher schedules a class, it will appear here.',
  'A IA ainda precisa de mais relatórios para medir a evolução com segurança.': 'AI still needs more reports to measure progress safely.',
});
...
function preserveCase(source: string, translated: string) {
  ...
  if (source === source.toUpperCase() && /[A-ZÀ-Ý]/i.test(source)) {
...
function replaceExpression(text: string, source: string, target: string, wordsOnly = false) {
  const escaped = escapeRegex(source);
  const pattern = wordsOnly
  ...
  return text.replace(pattern, (...args) => {
    if (wordsOnly) {
      const prefix = args[1] || '';
      const match = args[2] || '';
      return `${prefix}${preserveCase(match, target)}`;
    }
    const match = args[0] || '';
...
function translateText(original: string, language: string) {
  ...
  for (const [source, target] of fallbackEntries) {
    translated = replaceExpression(translated, source, target, true);
  }
  return translated;
}
